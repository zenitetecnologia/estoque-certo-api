...
public class UnidadeOrganizacionalRepository
{
    ...
    public async Task<List<UnidadeOrganizacional>> ObterUnidades()
    {
        const string sql = @"
            SELECT
                ...
                email
            FROM
                inventario_vegetal.unidades_organizacionais
            ORDER BY
                razao_social;
        ";
        ...
        try
        {
            ...
            while (await reader.ReadAsync())
            {
                unidades.Add(new UnidadeOrganizacional
                {
                    ...
                    Cpnj = reader.IsDBNull(reader.GetOrdinal("cpnj")) ? string.Empty : reader.GetString(reader.GetOrdinal("cpnj")),
    ...
    {
        const string sql = @"
            INSERT INTO inventario_vegetal.unidades_organizacionais 
            (
                id_ou_matriz,
                cpnj,
                razao_social,
        ...
        try
        {
            ...
            cmd.Parameters.AddWithValue("cpnj", unidade.Cpnj);
    ...
    public async Task<bool> VerificaExisteUnidade(string cpnj, int ignoreId)
    {
        const string sql = @"
            SELECT
                1
        ...
        try
        {
            ...
            await using var cmd = new NpgsqlCommand(sql, _connection);
            cmd.Parameters.AddWithValue("cpnj", cpnj);
